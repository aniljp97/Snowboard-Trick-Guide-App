...
public class ToDoFragment extends Fragment implements RecyclerViewClickListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view_root = inflater.inflate(R.layout.fragment_to_do_list, container, false);
        setHasOptionsMenu(true);

        trick_recyclerview = (RecyclerView) view_root.findViewById(R.id.todo_recyclerview);
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == addTrickRequestCode) {
        ...
        if(requestCode == editTrickRequestCode) {
            if(resultCode == RESULT_OK) {
        ...
        if(requestCode == addTrickManuallyRequestCode) {
            if(resultCode == RESULT_OK) {
                TrickData new_trick_data = (TrickData) data.getSerializableExtra(getString(R.string.updated_trick_key));
                addTrick(new_trick_data);
            }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch(item.getItemId()) {
            ...
            case R.id.action_delete:
                ...
                break;
            case R.id.action_move_to_completelist:
                moveToCompleteAreYouSureDialog();
                break;
    ...
    private void moveToCompleteAreYouSureDialog() {
        AlertDialog.Builder dialog = new AlertDialog.Builder(getContext());
        dialog.setMessage("Are you sure you want to move the "+trick_adapter.getSelectedCount()+ " selected tricks to your Tricks Completed List?");
        dialog.setPositiveButton(getString(R.string.yes_button_text), new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                for(TrickData trick : trick_adapter.getSelectedItems())
    ...
    private void setActionBarEditMode() {
        ...
        menu.findItem(R.id.action_move_to_completelist).setVisible(true);
    ...
    private void setTrickRecyclerView() {
        RecyclerView trick_recyclerview = (RecyclerView) view_root.findViewById(R.id.todo_recyclerview);
    ...
    private void goToAddTrickManualActivity() {
        Intent intent = new Intent(getActivity(), EditorActivity.class);
        intent.putExtra(getString(R.string.is_completed_key), 0);
